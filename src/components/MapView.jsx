    ...
    <div className="bg-gray-50 min-h-screen flex flex-col pb-32 animate-in fade-in duration-500">
      ...
      <div className="relative w-full aspect-square bg-white overflow-hidden border-b border-gray-200">
        ...
        <div className="absolute inset-0 grid grid-cols-3 grid-rows-3">
          {/* 左上 */}{' '}
          <div className="border-r border-b border-gray-100 bg-gray-50/30"></div>
          {/* 中上 */} <div className="border-r border-b border-gray-100"></div>
          {/* 右上 */}{' '}
          <div className="border-b border-gray-100 bg-gray-50/30"></div>
          {/* 左中 */} <div className="border-r border-b border-gray-100"></div>
          {/* 中心 */}{' '}
          <div className="border-r border-b border-gray-100 bg-blue-50/10"></div>
          {/* 右中 */} <div className="border-b border-gray-100"></div>
          {/* 左下 */}{' '}
          <div className="border-r border-gray-100 bg-gray-50/30"></div>
          {/* 中下 */} <div className="border-r border-gray-100"></div>
          {/* 右下 */} <div className="bg-gray-50/30"></div>
        </div>

        {/* --- 軸ラベル (端に配置) --- */}
        <div className="absolute top-2 left-1/2 -translate-x-1/2 text-xs font-bold text-gray-400 bg-white/80 px-2 py-0.5 rounded-full z-10">
          華やか (Aroma)
        </div>
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 text-xs font-bold text-gray-400 bg-white/80 px-2 py-0.5 rounded-full z-10">
          穏やか (Quiet)
        </div>
        <div className="absolute left-0 top-1/2 -translate-y-1/2 -rotate-90 text-xs font-bold text-gray-400 bg-white/80 px-2 py-0.5 rounded-full z-10">
          甘・旨
        </div>
        <div className="absolute right-0 top-1/2 -translate-y-1/2 rotate-90 text-xs font-bold text-gray-400 bg-white/80 px-2 py-0.5 rounded-full z-10">
          辛・キレ
        </div>
        ...
        {displayData.map((item) => {
          ...
          if (aiType.id === 'SOSHU') {
            ...
            labelBg = 'bg-blue-50 text-blue-800 border-blue-100';
          }
          if (aiType.id === 'JUNSHU') {
            borderColor = 'border-orange-500';
            labelBg = 'bg-orange-50 text-orange-800 border-orange-100';
          }
          if (aiType.id === 'KUNSHU') {
            borderColor = 'border-pink-500';
            labelBg = 'bg-pink-50 text-pink-800 border-pink-100';
          }
          if (aiType.id === 'JUKUSHU') {
            borderColor = 'border-amber-800';
            labelBg = 'bg-amber-50 text-amber-900 border-amber-100';
          }
          if (aiType.id === 'SHOCHU') {
            borderColor = 'border-purple-600';
            labelBg = 'bg-purple-50 text-purple-900 border-purple-100';
          }
          ...
          return (
            ...
            >
              {/* アイコン */}
              <div
                className={`w-10 h-10 md:w-12 md:h-12 rounded-full border-[3px] overflow-hidden shadow-lg bg-white ${borderColor}`}
              >
                {displayImage ? (
                  <img
                    src={displayImage}
              ...
              {/* 名前ラベル (視認性向上: 枠付き・影付き・改行なし) */}
              <span
                className={`text-[10px] font-bold px-2 py-0.5 rounded-full shadow-md border mt-[-6px] relative z-30 whitespace-nowrap max-w-[100px] truncate ${labelBg}`}
              >
                {item.name}
              </span>
            </div>
      ...
      <div className="p-4 space-y-4">
          ...
          {mapType === 'Sake' ? (
            <>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-bold text-gray-600">爽酒</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-full bg-orange-500"></div>
                <span className="text-xs font-bold text-gray-600">醇酒</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-full bg-pink-500"></div>
                <span className="text-xs font-bold text-gray-600">薫酒</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-full bg-amber-800"></div>
                <span className="text-xs font-bold text-gray-600">熟酒</span>
              </div>
            </>
          ) : (
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full bg-purple-600"></div>
              <span className="text-xs font-bold text-gray-600">焼酎</span>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
          <div className="bg-gray-50 px-4 py-3 flex items-center gap-2 border-b border-gray-100">
            ...
            <span className="text-sm font-bold text-gray-700">提案ナビ</span>
          </div>
          ...
          {activeTheme && (
            <div
              className={`mx-4 mb-4 p-4 rounded-xl text-sm leading-relaxed animate-in slide-in-from-top-2 duration-300 ${
                activeTheme.color
                  .replace('text-', 'bg-')
                  .replace('border-', '')
                  .split(' ')[0]
